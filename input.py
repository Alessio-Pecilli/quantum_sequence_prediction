...
def generate_quantum_dynamics_dataset(
    ...
    Protocollo:
      - Il tipo di Hamiltoniana è fissato (h_type).
      - Si estraggono B set di parametri casuali → B Hamiltoniane diverse dello stesso tipo.
    ...
    così da ottenere Hamiltoniane e stati iniziali completamente indipendenti.
    ...
    for b in range(B):
        ...
        H_b = _build_hamiltonian(h_type, n_qubits, h_params)
        
        # 2. Diagonalizzazione: H = V diag(E) V†  (eigh perché H è Hermitiana)
        #    Costo O(dim³) una tantum, poi evoluzione diventa banale.
        E_b, V_b = torch.linalg.eigh(H_b)
        V_b_dag = V_b.conj().T  # V†, per proiettare nella base degli autostati
        
        # 3. Fasi di evoluzione nella base diagonale per tutti i time-step
        #    phase(t) = exp(-i * E * dt * t),  shape (seq_len+1, dim)
        times = torch.arange(seq_len + 1, dtype=torch.float32).unsqueeze(1)  # (seq_len+1, 1)
        all_phases = torch.exp(-1j * E_b.unsqueeze(0) * dt * times)          # (seq_len+1, dim)
        
        for s in range(S):
            # 4. Estrazione stato iniziale casuale (normalizzato)
            ...
            # 5. Proiezione nella base degli autostati: c = V† |ψ>
            coeffs = V_b_dag @ psi  # (dim,)
            
            # 6. Evoluzione vettorizzata nella base diagonale
            #    c(t) = c(0) * exp(-i E dt t)  — moltiplicazione element-wise, O(dim) per step
            all_coeffs = coeffs.unsqueeze(0) * all_phases  # (seq_len+1, dim)
            
            # 7. Ritorno alla base computazionale: |ψ(t)> = V c(t)
            #    trajectory[t] = V @ all_coeffs[t]  →  batch: all_coeffs @ Vᵀ
            trajectory_tensor = all_coeffs @ V_b.T  # (seq_len+1, dim)
...
# ============================================================
# TEST STANDALONE — esegui con: python input.py
# ============================================================
if __name__ == "__main__":
    ...
    def check_dataset(name, inputs, targets, B, S, seq_len, n_qubits):
        ...
        assert inputs.shape == (total, seq_len, dim), f"Shape inputs errata!"
        assert targets.shape == (total, seq_len, dim), f"Shape targets errata!"
        print(f"    ✓ OK")
        ...
        print(f"    ✓ OK (complex64)")

        # --- Normalizzazione: ogni stato deve avere norma ≈ 1 ---
        ...
        print(f"    Norma inputs  — min: {norms_in.min():.6f}, max: {norms_in.max():.6f}, media: {norms_in.mean():.6f}")
        print(f"    Norma targets — min: {norms_tgt.min():.6f}, max: {norms_tgt.max():.6f}, media: {norms_tgt.mean():.6f}")
        ...
        print(f"    Campione di {len(idxs)} traiettorie: {'✓ coerente' if match else '✗ INCOERENTE!'}")
        assert match, "Input e target non sono temporalmente coerenti!"

        # --- Unitarietà: |<psi(t)|psi(t)>| = 1, overlap tra step vicini ---
        ...
        print(f"    Ampiezze — min: {amps.min():.4f}, max: {amps.max():.4f}, media: {amps.mean():.4f}")
        print(f"    Fasi     — min: {phases.min():.4f}, max: {phases.max():.4f}, media: {phases.mean():.4f}")
        ...
        print(f"    NaN: {'✗ TROVATI' if has_nan else '✓ nessuno'}")
        print(f"    Inf: {'✗ TROVATI' if has_inf else '✓ nessuno'}")
        assert not has_nan, "Trovati valori NaN!"
        assert not has_inf, "Trovati valori Inf!"

        print(f"\n  ✓ TUTTI I CHECK SUPERATI per {name}\n")

    # ---- Parametri di test (piccoli per velocità) ----
    ...
    print(f"  Train: B={B_tr}, S={S_tr} → {B_tr*S_tr} campioni")
    print(f"  Test:  B={B_te}, S={S_te} → {B_te*S_te} campioni")
    ...
    print(f"  ✓ DataLoader funzionante\n")
